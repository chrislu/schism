...
public:
    ...
    , _image_data_size(0)
    {}
    dds_data(const scm::shared_array<scm::uint8>&   rd,
             const DDS_HEADER*const                 ddsh,
             const DDS_HEADER_DXT10*const           ddsh10,
             const scm::uint8*const                 id,
             const scm::size_t                      ids)
    ...
    const DDS_HEADER*const          dds_header() const       { assert(_dds_header);       return _dds_header; }
    const DDS_HEADER_DXT10*const    dds_header_dxt10() const { assert(_dds_header_dxt10); return _dds_header_dxt10; }
    const scm::uint8*const          image_data() const       { assert(_image_data);       return _image_data; }
    scm::size_t                     image_data_size() const  {                            return _image_data_size; }

private:
    scm::shared_array<scm::uint8>   _raw_data;
    const DDS_HEADER*               _dds_header;
    const DDS_HEADER_DXT10*         _dds_header_dxt10;
    const scm::uint8*               _image_data;
    scm::size_t                     _image_data_size;
};

bool
read_dds_data_file(const std::string&   in_file_name,
                         dds_data&      out_dds_data)
{
    using namespace scm;
    using namespace scm::gl;
    using namespace scm::io;
    ...
    if (!f->open(in_file_name, std::ios_base::in, false)) {
        glerr() << log::error
                << "read_dds_data_file(): error opening file: "
                << in_file_name << log::end;
        return false;
    }

    if (f->size() < sizeof(DDS_HEADER) + sizeof(unsigned int)) {
        glerr() << log::error
                << "read_dds_data_file(): error file to small to hold header data: "
                << in_file_name << log::end;
        return false;
    }
    ...
    try {
        raw_data.reset(new scm::uint8[f->size()]);
    }
    catch(const std::bad_alloc& e) {
        glerr() << log::error
                << "read_dds_data_file(): error allocating image data store "
                << "(" << e.what() << "): "
                << in_file_name << log::end;
        return false;
    }
    ...
    // read file contents
    if (f->read(raw_data.get(), 0, f->size()) != f->size()) {
        glerr() << log::error
                << "read_dds_data_file(): error reading from file: "
                << in_file_name 
                << " (number of bytes attempted to read: " << f->size() << ")" << log::end;
        return false;
    }
    ...
    // check  magic number ("DDS ")
    unsigned int magic_number = *(unsigned int*)(raw_data.get());
    if (magic_number != DDS_MAGIC) {
        glerr() << log::error
                << "read_dds_data_file(): error file not starting with magic \"dds \" number: "
                << in_file_name << log::end;
        return false;
    }
    ...
    if (   dds_header->dwSize       != sizeof(DDS_HEADER)
        || dds_header->ddspf.dwSize != sizeof(DDS_PIXELFORMAT)) {
        glerr() << log::error
                << "read_dds_data_file(): error validating header size information: "
                << in_file_name << log::end;
        return false;
    }
    ...
    if (dds_header->ddspf.dwFourCC == SCM_MAKEFOURCC('D', 'X' ,'1' ,'0')) {
        ...
        if(f->size() < sizeof(DDS_HEADER) + sizeof(unsigned int) + sizeof(DDS_HEADER_DXT10)) {
            glerr() << log::error
                    << "read_dds_data_file(): error file to small to hold dxt10 header data as indicated by FOURCC \"DX10\": "
                    << in_file_name << log::end;
            return false;
        }
