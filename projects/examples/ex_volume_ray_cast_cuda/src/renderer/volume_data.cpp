                         ...
                         const std::string&           in_file_name)
{
    ...
    //vec3ui data_offset = vec3ui(vol_reader->dimensions().x / 2, vol_reader->dimensions().y / 2, 0u);//vol_reader->dimensions() / vec3ui(2, 2, 2);
    //data_dimensions    = vol_reader->dimensions() / vec3ui(5, 5, 1);
    vec3ui data_offset = vec3ui(0);//vol_reader->dimensions().x / 2, vol_reader->dimensions().y / 2, vol_reader->dimensions().z / 2);//vol_reader->dimensions() / vec3ui(2, 2, 2);
    data_dimensions    = vol_reader->dimensions();// / vec3ui(1, 1, 5);
                                          ...
                                          unsigned               in_size) const
{
    ...
    out() << "allocating texture storage ("
          ...
          << log::end;
    timer.start();
    texture_2d_ptr new_tex = in_device->create_texture_2d(vec2ui(in_size, 1), FORMAT_RGBA_8, 1, 1);
    timer.stop();
    out() << "allocating texture storage done."
          << " (elapsed time: " << std::fixed << std::setprecision(3)
          << time::to_seconds(timer.get_time()) << "s)" << log::end;
