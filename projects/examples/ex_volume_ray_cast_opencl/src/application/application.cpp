...
#include <renderer/opencl_volume_renderer.h>
...
application_window::init_renderer()
{
    ...
    if (!_viewer->device()->enable_opencl_interop()) {
        err() << log::error
              << "application_window::init_renderer(): unable to initialize OpenCL system."
              << log::end;
        return false;
    }

    _cl_device = _viewer->device()->opencl_interop_device();
    _cl_queue  = _viewer->context()->opencl_command_queue();

    //try {
    //    _cl_device.reset(new cl::opencl_device());
    //    _cl_queue = _cl_device->cl_main_command_queue();

    //    out() << *_cl_device << log::end;
    //}
    //catch (std::exception& e) {
    //    std::stringstream msg;
    //    msg << "application_window::init_renderer(): unable to initialize OpenCL system ("
    ...
    try {
        ...
        //_volume_data_opencl.reset(new opencl_volume_data(device, _volume_data));

        _volume_renderer.reset(new volume_renderer(device, _viewport_size));
        _volume_renderer_opencl.reset(new opencl_volume_renderer(device, _cl_device, _viewport_size));
    }
    catch (std::exception& e) {
        std::stringstream msg;
                           ...
                           const gl::render_context_ptr& context)
{
    ...
    if (_volume_data) {
        ...
        _volume_data_opencl->update(context, _cl_queue);
    }
}
    ...
    { // draw volumes
        ...
        if (_volume_data) {
            ...
            if (_use_opencl_renderer) {
                _volume_renderer_opencl->draw(context, _cl_queue, _volume_data_opencl);
                _volume_renderer_opencl->present(context);
            }
            else {
    ...
    if (_volume_data) { // text overlay
        vec3ui lod_size = util::mip_level_dimensions(_volume_data->data_dimensions(), static_cast<unsigned>(_volume_data->selected_lod()));
        {
            ...
            os << std::fixed << std::setprecision(3)
               << (_use_opencl_renderer ? "OpenCL " : "OpenGL ")
    ...
    if (state) { // only fire on key down
        switch(k) {
            ...
            case Qt::Key_S:         _volume_renderer->reload_shaders(_viewer->device());
                                    _volume_renderer_opencl->reload_kernels(_viewer->device(), _cl_device);
...
application_window::open_volume()
{
    ...
    if (!image_file.empty()) {
        ...
        try {
            ...
            _volume_data_opencl.reset(new opencl_volume_data(_viewer->device(), _cl_device, _volume_data));
        }
        catch (std::exception& e) {
