...
private:
    ...
    scm::gl::texture_2d_ptr             _color_buffer_resolved_view;
    ...
    scm::gl::program_ptr                _compute_shader_prg;
...
demo_app::initialize()
{
    ...
    if (   !io::read_text_file("../../../src/shaders/phong_lighting.glslv", vs_source)
        ...
        || !io::read_text_file("../../../src/shaders/compute_shader.glslc", cs_source)        
        ) {
    ...
    _compute_shader_prg = _device->create_program(list_of(_device->create_shader(STAGE_COMPUTE_SHADER, cs_source))                                                     
                                                     );
    ...
    _device->create_texture_2d(scm::math::vec2ui(512), FORMAT_R_32F);
    ...
    //_cs_texture_image_2d = _device->b(vec2ui(512,512), FORMAT_R_8);
        
        /*texture_2d_desc(vec2ui(512,512), data_format::FORMAT_R_8));*/
...
demo_app::display()
{
...

    {
        ...
        _compute_shader_prg->uniform_sampler("in_texture_RGBA8", 0);
        _pass_through_shader->uniform("in_texture_RG16", 1);
        _compute_shader_prg->uniform("roll", (float)frame * 0.1f);
    
        //_compute_shader_prg->bind(*(_context.get()));
        _context->bind_program(_compute_shader_prg);

        _context->bind_texture(_color_buffer_resolved, _filter_nearest, 0);
        _context->bind_image(_color_buffer_resolved, _color_buffer_resolved->format(), access_mode::ACCESS_READ_WRITE, 0);

        //_context->bind_texture(_color_buffer_resolved_view, _filter_nearest, 1);
        //_context->bind_image(_color_buffer_resolved_view, _color_buffer_resolved_view->format(), access_mode::ACCESS_READ_ONLY, 1);
        ...
        if (gl_error != GL_NO_ERROR) {
            std::cout << "bug after compute shader" << gl_error << std::endl;
        }        
    }
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("simple_glut_compute");
