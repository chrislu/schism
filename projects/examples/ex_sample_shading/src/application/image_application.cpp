...
application_window::init_renderer()
{
    ...
    _rstate_cback_sshading   = device->create_rasterizer_state(FILL_SOLID, CULL_BACK, ORIENT_CCW, true, true, 1.0f);
    ...
    { // this way for texture
        ...
        _texture_alpha = tl.load_texture_2d(*device, "../../../res/textures/0001MM_disp.jpg", true, false);

        if (!_texture || !_texture_alpha) {
            err() << "application_window::init_renderer(): error loading texture images." << log::end;
            return false;
        }
    }
...
application_window::shutdown()
{
    ...
    _dstate_less.reset();
    _rstate_cback.reset();
    _rstate_cback_sshading.reset();
    _sstate_linear.reset();
    
    _camera_block.reset();
    
    _texture.reset();
    _texture_alpha.reset();
...
application_window::display(const gl::render_context_ptr& context)
{
    ...
    { // image rendering pass
        ...
        context->set_rasterizer_state(_use_sample_shading ? _rstate_cback_sshading : _rstate_cback);
...
application_window::keyboard_input(int k, bool state, scm::uint32 mod)
{
    if (state) { // only fire on key down
        switch(k) {
            ...
            case Qt::Key_Space:     _use_sample_shading = !_use_sample_shading; break;
