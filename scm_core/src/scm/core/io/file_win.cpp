...
{
    overlapped_ext(const file_win::size_type size);
    ~overlapped_ext(); // needs to be non-virtual

    void                                            position(const file_win::size_type pos);
    file_win::size_type                             position() const;

    void                                            bytes_to_process(const file_win::size_type size);
    file_win::size_type                             bytes_to_process() const;

    const scm::shared_ptr<file_win::char_type>&     buffer() const;

private:
    file_win::size_type                             _bytes_to_process;
    scm::shared_ptr<file_win::char_type>            _rw_buffer;
...
overlapped_ext::overlapped_ext(const file_win::size_type size)
  : _bytes_to_process(0)
{
    hEvent = 0;
    _rw_buffer.reset(static_cast<file_win::char_type*>(VirtualAlloc(0, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)),
...
void
overlapped_ext::position(const file_win::size_type pos)
{
    LARGE_INTEGER   l;
    l.QuadPart = pos;
...
void
overlapped_ext::bytes_to_process(const file_win::size_type size)
{
...
const scm::shared_ptr<file_win::char_type>&
...
file_win::file_win(const file_win& rhs)
  : file_base(rhs),
    ...
    _completion_port(rhs._completion_port)
{
}
...
file_win::swap(file_win& rhs)
{
    ...
    std::swap(_completion_port,     rhs._completion_port);
}
...
file_win::size_type
file_win::read(char_type*const output_buffer,
...
file_win::size_type
file_win::write(const char_type*const input_buffer,
...
file_win::size_type
file_win::read_async(char_type*const output_buffer,
                     size_type       num_bytes_to_read)
{
            ...
            foreach (const detail::io_result& result, results) {
                ...
                if (result._bytes_processed != result._ovl->bytes_to_process()) {
                    if (result._ovl->position() + result._bytes_processed < _file_size) {
                        scm::err() << scm::log_level(scm::logging::ll_error)
                                   << "file_win::read_async(): read result with different than requested length "
                ...
                if (memcpy(copy_dst, copy_src, copy_read_bytes) != copy_dst) {
                    scm::err() << scm::log_level(scm::logging::ll_error)
                               << "file_win::read_async(): error copying to destination buffer" << std::endl;
                ...
                if (result_request != running_requests.end()) {
                    ...
                    scm::err() << scm::log_level(scm::logging::ll_error)
                               << "file_win::read_async(): error finding result read request in running request list" << std::endl;
...
bool
file_win::read_async_request(const detail::request_ptr& req) const
{
    DWORD request_bytes_read = 0;
...
file_win::size_type
file_win::write_async(const char_type*const input_buffer,
                      size_type             num_bytes_to_write)
{
    assert(async_io_mode());
    ...
    if (begin_sec_prefetch != 0 &&
        end_sec_prefetch   != 0) {
        scm::err() << scm::log_level(scm::logging::ll_error)
                   << "file_win::write_async(): trying to write shit that is not vss aligned!"
    ...
    do {
        ...
        while (!free_requests.empty() && next_write_request_pos < position_end_vss) {
            ...
            if (memcpy(copy_dst, copy_src, copy_write_bytes) != copy_dst) {
                scm::err() << scm::log_level(scm::logging::ll_error)
                           << "file_win::write_async(): error copying to destination buffer" << std::endl;
        ...
        if (!running_requests.empty()) {
            ...
            foreach (const detail::io_result& result, results) {
                if (result._bytes_processed != result._ovl->bytes_to_process()) {
                    scm::err() << scm::log_level(scm::logging::ll_error)
                               << "file_win::write_async(): write result with different than requested length "
                               << "(requested: " << result._ovl->bytes_to_process()
                               << ", read: " << result._bytes_processed << ")" << std::endl;
                ...
                if (result_request != running_requests.end()) {
                    ...
                    scm::err() << scm::log_level(scm::logging::ll_error)
                               << "file_win::write_async(): error finding result write request in running request list" << std::endl;
...
bool
file_win::write_async_request(const detail::request_ptr& req) const
{
...
bool
file_win::query_async_results(std::vector<detail::io_result>& results_vec,
                              int query_max_results) const
{
                                    ...
                                    FALSE) != 0)
    {
        ...
        scm::err() << scm::log_level(scm::logging::ll_error)
                   << "file_win::read_async(): GetQueuedCompletionStatusEx returned with error" << std::endl;
                                  ...
                                  INFINITE) != 0)
    {
        ...
        scm::err() << scm::log_level(scm::logging::ll_error)
                   << "file_win::read_async(): GetQueuedCompletionStatus returned with error" << std::endl;
