...
{
    vec4 res;
    vec3 n = gl_FrontFacing ? normalize(normal) : normalize(-normal); //normalize(normal);//
    ...
    res =  gl_FrontLightProduct[0].ambient
         + gl_FrontLightProduct[0].diffuse * (dot(n, l) * 0.5 + 0.5)//gl_FrontLightProduct[0].diffuse * max(0.0, dot(n, l))//
