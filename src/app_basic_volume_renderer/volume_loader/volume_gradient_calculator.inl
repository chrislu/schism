...
#include "volume_filter_kernel_sobel3x3.h"
    ...
    for (unsigned int z = 0; z < depth; z++) {
        for (unsigned int y = 0; y < height; y++) {
            for (unsigned int x = 0; x < width; x++) {
                ...
                tmp_dx = sobel_x.apply(cur_data);
                tmp_dy = sobel_y.apply(cur_data);
                tmp_dz = sobel_z.apply(cur_data);
                ...
                dst_buffer[get_output_offset(x, y, z) + 0] = tmp_dx;
                dst_buffer[get_output_offset(x, y, z) + 1] = tmp_dy;
                dst_buffer[get_output_offset(x, y, z) + 2] = tmp_dz;
